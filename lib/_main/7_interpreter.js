		...
		makeInterpreter = (function () {
			var internalInterpreter,
				...
				helpers = {
					...
					replaceToken: function (template, token, repl) {
						repl = repl === undefined || repl === null ? '' : repl.toString();
				...
				makeInterpreter = function () {
					return {
						...
						interpret: function (template, data, partials, delimiters, disableRecursion, contextStack) {
							return internalInterpreter.interpret({
								template: template,
								data: data,
								partials: partials,
			...
			internalInterpreter = {
				...
				// disableRecursion - [OPTIONAL] flag to disable recursive rendering.
				...
				interpret: function (args) {
					var template = makeMutableString(args.template ? args.template.toString() : ''),
						partials = args.partials || {},
						data = args.data === null || args.data === undefined ? {} : args.data,
